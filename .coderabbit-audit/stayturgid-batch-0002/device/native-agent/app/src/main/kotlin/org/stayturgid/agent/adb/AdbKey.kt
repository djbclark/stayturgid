...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    private fun decrypt(ciphertext: ByteArray, aad: ByteArray?): ByteArray? {
        ...
        // IV is not reused: encrypt() lets the Cipher generate a fresh random IV
        // per call (cipher.iv) and prepends it; decrypt() reads that unique IV
        // back. Standard AES-GCM, matching Shizuku's AdbKey.
    ...
    fun sign(data: ByteArray?): ByteArray {
        // Not block-cipher ECB: this is the RSA signing primitive the ADB
        // A_AUTH handshake requires — PKCS#1 v1.5 padding is applied manually
        // via PADDING, then the token is signed with raw RSA. Matches Shizuku.
        // nosemgrep: kotlin.lang.security.ecb-cipher.ecb-cipher
