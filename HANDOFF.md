...
- **Deploy the fleet:** `./mac/deploy-fleet.sh` (thin wrapper over
  `ansible/playbooks/fleet.yml`; `CHECK=1 ./mac/deploy-fleet.sh` = dry run).
  Both phones are currently deployed, idempotent (re-run = `changed=0`), and
  post-reboot-validated (bridge pidfile + watchdog relaunch confirmed).
- **Test:** `make test` (tier a syntax/lint + tier b shell TAP + pytest twins +
  `ansible-test units`; no device). `make verify` = read-only device tier.
  `make lint` = shellcheck/ansible-lint/yamllint. CI runs `make test` on push.
  First-time setup: `make test-venv` (builds `.venv-test`).
- **Where things live:** device-specific facts ONLY in
  `ansible/inventory/hosts.yml` + `inventory/group_vars/` taxonomy layers
  (android_16 → vendor_* → oneui_7 → model_*). The `termux_pkg` module is a
  collection: `ansible_collections/stayturgid/fleet/` (FQCN
  `stayturgid.fleet.termux_pkg`). Mac config generated by
  `ansible/playbooks/mac.yml` → `~/.config/stayturgid/devices.conf` + launchd.
- **⚠ Needs human hands (can't be done headlessly):**
  1. **p7a: fix `upmon` in Tasker GUI** — it still references the deleted
...
**"Parity soaks":** a rewritten script (e.g. bash→python) keeps BOTH
implementations deployed and run against the identical test suite
(`battery_suite sh|py`) until the twin has matched behavior in tests AND on real
hardware through real events — only then is the old one retired. Exit criterion,
not a vibe: same test results on both + observed-correct on-device.
