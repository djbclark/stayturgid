...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **K1 — native agent** (AutoJs6 → Kotlin/Shizuku UserService) | Cutover landed (195c5c7, 2026-07-22), **not fully verified** | Only s24 confirmed post-cutover. AutoJs6-uninstall task is `failed_when: false` (can't fail, so "uninstalled" is unproven on p7a/hd8). The `CLOSED_NO_SHELL` soak and 7-day OpenObserve soak that were supposed to gate "complete" never ran. Fleet last seen running the `.debug` APK, not a signed release. See [operations/sessions/handoff-2026-07-23-native-agent-k1.md](operations/sessions/handoff-2026-07-23-native-agent-k1.md).                                                                                      |
| **OpenObserve ↔ Vector auth**                                | **Broken, live**                                             | 401 Unauthorized on both OO sinks (latest seen 2026-07-23T12:21Z). Blocks all K1 soak evidence and both T5 follow-ons. Operator action: set `OPENOBSERVE_ROOT_EMAIL`/`OPENOBSERVE_ROOT_PASSWORD` for the Vector LaunchAgent, restart Vector, then run `control/tools/native-agent/reingest_soft_health.py` (dry-run first). `soft_health.jsonl` on the Mac is the source of truth until this is fixed.                                                                                                                         |
| **F1 — FIRERPA MCP bridge**                                  | Planned, not implemented                                     | Decisions D1–D3 finalized ([operations/plans/firerpa-mcp-bridge-plan-2026-07-22.md](operations/plans/firerpa-mcp-bridge-plan-2026-07-22.md)). `mcp` 1.28.1 installed in `~/.venv-stayturgid-firerpa` (that venv has **no pip binary** — verify with `python -c "import importlib.metadata; ..."`, not `pip show`). Implementation gated on: (1) operator "go", (2) a decision on the ⚑ consent-surface question (recommendation: v1 = local osascript consent, add MCP elicitation later once a remote tailnet caller exists). |
| **T5 — observability/portal unification**                    | Evaluated, not started                                       | Reject Homer/Glance ([research/evaluations/observability-portal-unification-evaluation-2026-07-23.md](research/evaluations/observability-portal-unification-evaluation-2026-07-23.md)). Two follow-ons (OO→Grafana Prometheus datasource; run the dashboard-framework research prompt) both blocked on the Vector 401 above.                                                                                                                                                                                                   |
| **Settings-state corruption** (issues #41, #42)              | Investigation open                                           | Battery-percentage reset + portrait-lock flip on Android; hypothesis is settings-DB corruption around sleep/wake. #16 describes an overlapping symptom on p7a and should be cross-linked.                                                                                                                                                                                                                                                                                                                                      |
...
1. Set OpenObserve credentials for the Vector LaunchAgent and restart it.
2. Physically check p7a and hd8 (Tailscale unreachable).
3. Decide the F1 consent-surface phasing question.
