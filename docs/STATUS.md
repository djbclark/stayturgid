...
| **K1 — native agent** (AutoJs6 -> Kotlin/Shizuku UserService)                                                                                    | Cutover landed (195c5c7, 2026-07-22), **still not fully verified — see 2026-07-25 correction** | Live-checked all three devices 2026-07-25: the cutover's "AutoJs6 uninstalled" claim was **false** — `pm path org.autojs.autojs6` returned a real installed APK on s24, hd8, and p7a (including s24, the one device the cutover session claimed to have confirmed). Uninstalled fleet-wide for real 2026-07-25; `pm path` now empty on all three and agent health (`shizuku=up sshd=up a11y=up shell=yes`) unaffected, including `a11y=up` with AutoJs6 gone, confirming the native agent has its own accessibility service. Also confirmed live: fleet is running `org.stayturgid.agent.debug` (`0.3.6-diagnostics-debug`), not the stale non-debug `org.stayturgid.agent` (`0.3.1`, untouched since 2026-07-22) — release build still not shipped, tracked in [#45](https://github.com/djbclark/stayturgid/issues/45). The `CLOSED_NO_SHELL` soak still hasn't run — now more load-bearing than before, since AutoJs6's removal means the soak would for the first time test the native agent's catastrophic-repair path with no fallback. [#43](https://github.com/djbclark/stayturgid/issues/43) tracks the remainder. See [operations/sessions/handoff-2026-07-23-native-agent-k1.md](operations/sessions/handoff-2026-07-23-native-agent-k1.md). |
