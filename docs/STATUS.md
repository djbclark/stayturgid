...
| **K1 — native agent** (AutoJs6 -> Kotlin/Shizuku UserService)                                                                                    | Cutover landed (195c5c7, 2026-07-22), **3 bugs fixed + deployed, 2 open, still not fully verified** | AutoJs6 genuinely uninstalled fleet-wide (2026-07-25, correcting a false cutover claim). Three real bugs found via reboot-based `CLOSED_NO_SHELL` soak attempts, all **fixed, built, and deployed fleet-wide this session, verified via live reboot capture**: (1) boot-time Shizuku-binder race in `HostService` (was a ~20min post-reboot detection blind spot), (2) `listeningOn()` false-positive on loopback-only binds, (3) USB debugging now proactively kept enabled every comonitor tick. **Two items remain open, not code-fixable this session:** the Fire-OS adbd-restart gap (confirmed via direct testing that no non-root, no-USB lever exists — physical/USB recovery is the accepted fallback, not a bug to keep chasing) and a broader finding that Shizuku (and once, Termux/sshd) die unexpectedly on **both** hd8 and s24, not just Fire OS — needs dedicated investigation before the soak test can be trusted. Full writeup: [operations/sessions/session-2026-07-25-k1-verification.md](operations/sessions/session-2026-07-25-k1-verification.md). Tracked in [#43](https://github.com/djbclark/stayturgid/issues/43); debug-vs-release gap in [#45](https://github.com/djbclark/stayturgid/issues/45). |
