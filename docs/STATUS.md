...
| **K1 — native agent** (AutoJs6 -> Kotlin/Shizuku UserService)                                                                                    | Cutover landed (195c5c7, 2026-07-22), **still not fully verified — real bug found** | AutoJs6 was actually still installed fleet-wide despite the cutover's claim otherwise — now genuinely uninstalled (2026-07-25). Reboot-based `CLOSED_NO_SHELL` soak attempts on s24/p7a found a real bug instead: `HostService`'s first post-boot comonitor check races Shizuku's binder connection and silently loses on a fixed 2s delay (confirmed via live logcat: `comonitor skipped — not bound` fires 2.5s before the binder is actually ready), then falls through to a 20-minute retry gap with **no `CLOSED_NO_SHELL` detection at all** during that window. Needs a code fix + rebuild + redeploy, not done yet. Full writeup: [operations/sessions/session-2026-07-25-k1-verification.md](operations/sessions/session-2026-07-25-k1-verification.md). Tracked in [#43](https://github.com/djbclark/stayturgid/issues/43); debug-vs-release gap in [#45](https://github.com/djbclark/stayturgid/issues/45). |
