# Session 2026-07-23 — AI memory policy migration to git (~/ops/site-private)
...
`docs/STATUS.md` (this repo), and `~/ops/site-private/README.md` for the
...
- **`~/ops/stayturgid`** (public) HEAD `e4b402c`. Added `docs/notes/lessons-learned.md`
...
- **`~/ops/site-djbclark`** (private) HEAD `f966ca2`. Got its first `AGENTS.md`
...
- **`~/ops/site-private`** (new, private) HEAD `1ec78cf`. Created fresh this
...
- `~/AGENTS.md` → `~/ops/site-private/home-agents.md`
- `~/CLAUDE.md` → `~/ops/site-private/home-agents.md` (points directly at the
...
- `~/.claude/projects/-Users-djbclark-src-ai/memory/` — `~/src/ai` (GitHub
  ...
  session doc and in `~/ops/site-private/README.md`'s policy).
...
- `~/ops/site-private` is private on GitHub (`djbclark/site-private`) — if it
