...
- `docs/modules/termux.md` standalone section runs `pkg update && pkg upgrade -y` twice in a row (lines ~30-31).
- `docs/modules/termux.md` says the torch pulse "count matches tier" from 15% — matches code (`pulse_torch "$blinks"`), but the DND behavior line omits that the _notification_ is still posted (silently) in quiet mode; minor drift from the script header, which does say it.
