...
**Location:** `docs/research/experiments/` (speculative). Production-adjacent UI/ADB notes
...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **shell-gpt + cloud API** (OpenAI/Anthropic) | Best _quality_ for rare escalation; needs network + key; cost per call                                       |
...
**Mac vision gates (UI-TARS)** are separate from on-device LLM: see [docs/architecture/vlm.md](../../docs/architecture/vlm.md).
