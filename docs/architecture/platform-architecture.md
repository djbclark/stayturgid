...
| --- | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| 1   | Private site overlay repo + access model          | **DECIDED 2026-07-18:** GitHub private `site-djbclark`, one repo for the whole site (stayturgid overlay + general machine management). Base-dir convention `${OPS_ROOT:-~/ops}` with sibling checkouts. See the site repo's `docs/plans/site-djbclark-step1-segmentation-architecture-v1.md`.                     | Security, backup, CI           |
...
| 9   | O-V-G-O port assignments and Caddy routing        | **HALF-DECIDED 2026-07-18:** the site repo's `registry/ports.yml` is the port authority (Grafana 3000, VictoriaMetrics 8428, OliveTin 1337 registered there); daemon _instances_ are site-owned with stayturgid as a tenant via serverapp adapter roles (site repo step1 doc §5a). Remaining: Caddy route naming. | Network layout, firewall rules |
