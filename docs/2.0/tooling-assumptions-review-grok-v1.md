...
2.0 changes _placement_ (obs-main off the sleeping M1 Air → VPS role), not _tool choice_. OliveTin must remain a thin button face over existing just/Ansible verbs so it does not become a second unsigned remote-exec path. OpenObserve clean-log soak remains an ops gate, not a redesign trigger.
