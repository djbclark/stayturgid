...
check-ts:
    ...
    @for f in $(find just/tools docs/research -name "*.ts" -not -name "*.d.ts" -not -path "*/node_modules/*" 2>/dev/null); do \
      js_file="${f%.ts}.js"; \
      if [ ! -f "$js_file" ]; then \
    ...
    @for f in $(find just/tools docs/research -name "*.js" -not -path "*/node_modules/*" 2>/dev/null); do \
      ts_file="${f%.js}.ts"; \
      if [ ! -f "$ts_file" ]; then \
