...
/**
 * Static peer-start assignment: which Fire-OS devices *this* device is responsible for starting
 * Shizuku on, over external ADB (issue #61).
 *
 * The fleet is small, so assignment is static config rather than dynamic discovery. The config is
 * app-readable (never a secret — just `host:port` targets) and is provisioned out of band (ansible
 * / adb). Search order:
 * 1. internal `filesDir/peer.json` (provisioned via `run-as … cp`)
 * 2. external `getExternalFilesDir/peer.json` (provisioned via `adb push`) First file found wins.
 *    Absent/empty ⇒ empty target list ⇒ the peer-start loop is a no-op (hd8 itself carries no
 *    config; s24/p7a list hd8).
 *
 * Schema:
 * ```json
 ...
 * ```
 */
data class PeerConfig(val targets: List<PeerTarget>, val shizukuPkg: String) {
    companion object {
        ...
        fun candidatePaths(context: Context): List<File> = buildList {
        ...
        fun parse(json: String): PeerConfig {
            ...
            val pkg =
                obj.optString("shizuku_pkg", DEFAULT_SHIZUKU_PKG).ifBlank { DEFAULT_SHIZUKU_PKG }
