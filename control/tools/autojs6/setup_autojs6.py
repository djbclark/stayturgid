...
def deploy_termux_scripts(alias: str, serial: str) -> str:
    ...
    termux = REPO_ROOT / "device" / "termux"
...
def main(argv: list[str] | None = None) -> int:
    ...
    harden = REPO_ROOT / "control" / "bin" / "harden_fleet_apps.py"
    if harden.is_file():
        print("Hardening fleet app permissions and battery settings...")
        subprocess.run([sys.executable, str(harden), alias], check=False)

    sync = REPO_ROOT / "control" / "tools" / "obtainium" / "sync_to_device.py"
    enable = TOOL_DIR / "enable_autojs6_shizuku.py"
    if sync.is_file():
