...
Deterministic order:
  ...
  4. Fleet drawer profile from control/lib/autojs6_drawer_defaults.json (UI)
...
DRAWER_DEFAULTS = REPO_ROOT / "control" / "lib" / "autojs6_drawer_defaults.json"
...
def apply_drawer_defaults(serial: str) -> list[str]:
    """Apply control/lib/autojs6_drawer_defaults.json via drawer UI. Returns failure labels."""
...
def enable_accessibility(serial: str, alias: str) -> bool:
    ...
    if lost:
        sys.stderr.write(
            "ERROR: accessibility list shrank (%s) — run ./control/bin/a11y_services.py restore %s\n"
            % (", ".join(lost), alias)
        )
