...
Serves on port 8080. Reads services catalog from services.json (generated by
discover.py). Supports HTMX-based hide/show for unreachable services and
on-demand rescan.
"""
...
_SERVICES_FILE = _HERE / "services.json"
...
def _load_services() -> list[dict]:
    if _SERVICES_FILE.is_file():
        try:
            return json.loads(_SERVICES_FILE.read_text()).get("services", [])
...
def _load_hidden() -> list[str]:
    if _SERVICES_FILE.is_file():
        try:
            return json.loads(_SERVICES_FILE.read_text()).get("hidden", [])
        except (json.JSONDecodeError, OSError):
            pass
    return []
...
def _load_last_scan() -> str:
    if _SERVICES_FILE.is_file():
        try:
            return json.loads(_SERVICES_FILE.read_text()).get("last_scan", "")
        except (json.JSONDecodeError, OSError):
            pass
    return ""
...
def _save_hidden(hidden: list[str]) -> None:
    ...
    _SERVICES_FILE.parent.mkdir(parents=True, exist_ok=True)
    try:
        _SERVICES_FILE.write_text(json.dumps(data, indent=2, sort_keys=True))
    except OSError:
        pass
